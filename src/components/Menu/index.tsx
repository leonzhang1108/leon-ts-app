...
import batmanSvg from '@img/batman.svg'
import './index.less'
...
const Leon = (props: any) => {
  ...
  useEffect(() => {
    ...
    if (index % 3 !== 0) {
      ...
      const scale = width / bgCtx.measureText(remainder === 1 ? 'DESHI' : 'BASARA').width
      const fontSize = Math.min(MAX_FONT_SIZE, MAX_FONT_SIZE * scale * 1)
      bgCtx.font = `${fontWeight} ${fontSize}px ${fontFamily}`
      bgCtx.fillText(remainder === 1 ? 'DESHI' : 'BASARA', width / 2, height / 2)
      getMatrix()
    } else {
      const img = new Image()
      img.onload = function () {
  ...
  return (
    <div className="leon-wrapper" onClick={canvasOnClick}>
      <canvas
        style={canvasStyle}
        ref={canvasRef}
        height={height}
